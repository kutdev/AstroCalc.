...
round.addEventListener("click", function(){
    ...
    if (newNumber.indexOf(".")>-1) {
       ...
       const newControlNumberRound = String(slicedNumber).slice(0, -dotLength+1);
       if (newControlNumberRound > 4 && newControlNumberRound < 10) {
        newWord = newNumber.slice(0, -dotLength-1);
        resultRounded = parseFloat(newWord);
        result.textContent = resultRounded += 1;
       }
       if (newControlNumberRound > -1 && newControlNumberRound < 5) {
        newWord = newNumber.slice(0, -dotLength-1);
        resultRounded = parseFloat(newWord);
